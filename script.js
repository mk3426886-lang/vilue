// 1. قاعدة بيانات المنتجات المبدئية (الأجهزة الذكية)
...
// رقم الهاتف الخاص بالمدير لإرسال الطلبات (الرقم الذي أرسلته)
const MANAGER_PHONE = "+9647708574713";

// جلب البيانات من التخزين المحلي للمتصفح (لتذكر الكميات والأكواد حتى لو أغلقت الموقع)
let products = JSON.parse(localStorage.getItem('lion_products')) || initialProducts;
let cart = [];
let activeCoupon = null;

// قاعدة بيانات أكواد الخصم المتاحة للمدير
...
function addToCart(id) {
    ...
    if (cartItem) {
        if (cartItem.qty < product.stock) {
            ...
            alert("عذراً، لقد وصلت للحد الأقصى المتاح من هذا المنتج!");
            return;
        }
    } else {
    ...
    cart.forEach(item => {
        ...
        itemRow.innerHTML = `
            <div>
                ...
                <small>${item.price.toLocaleString()} م.ي × ${item.qty}</small>
            </div>
...
function applyCoupon() {
    ...
    if (coupons[codeInput]) {
        ...
        if (coupon.used < coupon.limit) {
            ...
            alert("عذراً، هذا الكود انتهت عدد مرات استخدامه المسموحة!");
        }
    } else {
...
// فتح وإغلاق نوافذ إدخال البيانات (Modals)
...
// إرسال نموذج العميل وتوليد رقم الطلب الفريد
let currentOrderData = null; // لحفظ بيانات الطلب الحالي قبل إرساله للواتساب
...
function handleFormSubmit(event) {
    ...
    // توليد كود طلب فريد وذكي (LION - رقم عشوائي)
    ...
    // تحديث كميات المنتجات في المخزن (خصم المنتجات المطلوبة)
    ...
    // تحديث استخدام الكود إذا تم استخدامه
    ...
    // عرض الفاتورة للعميل في النافذة الثانية
    ...
    // إغلاق نافذة العميل وفتح نافذة الفاتورة والواتساب
    closeCheckoutModal();
    document.getElementById("receiptModal").classList.add("active");

    // تحديث واجهة المنتجات والسلة الرئيسية
    renderProducts();
    cart = [];
    activeCoupon = null;
    updateCartUI();
...
// إرسال الرسالة الاحترافية المنسقة إلى واتساب المدير
function sendToWhatsApp() {
    if (!currentOrderData) return;

    // بناء نص الرسالة بأسلوب مرتب جداً
    ...
    // تحويل النص إلى صيغة روابط الإنترنت الصالحة
    const encodedMessage = encodeURIComponent(message);
    
    // فتح رابط الواتساب مباشرة لرقم المدير المحدد
    const whatsappURL = `https://wa.me/${MANAGER_PHONE}?text=${encodedMessage}`;
    window.open(whatsappURL, '_blank');

    // إغلاق نافذة الفاتورة بعد الإرسال
...
function addNewCoupon() {
    ...
    if (!code || isNaN(percent) || isNaN(limit)) {
        alert("يرجى ملء جميع حقول كود الخصم بشكل صحيح!");
        return;
    }
    ...
    alert(`تمت إضافة كود الخصم [ ${code} ] بنجاح بنسبة ${percent}% لعدد ${limit} مستخدم.`);
...
// ملء قائمة الاختيار للمدير لتحديث كميات المنتجات
...
function addProductStock() {
    ...
    if (isNaN(addedQty) || addedQty <= 0) {
        alert("يرجى كتابة كمية صحيحة أكبر من الصفر.");
        return;
    }
    ...
    if (product) {
        ...
        renderProducts();
        alert(`تم تحديث المخزن بنجاح! الكمية الجديدة لـ (${product.name}) أصبحت: ${product.stock}`);
